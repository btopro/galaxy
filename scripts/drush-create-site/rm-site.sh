...
for sitedata in `find $galaxy/config/stacks/$2 -name $1 | grep -v services` ; do
    ...
    if [ $rmsitedata == "y" ] || [ $rmsitedata == "yes" ]; then
        ...
        echo "removing site data"
            servicestest=`find $galaxy/config/stacks/$2/sites/$2/services/ -name $2`
            echo "services test"
            echo $servicestest
...
#grep for coursename
cd $galaxy/config/stacks/$2/sites/
